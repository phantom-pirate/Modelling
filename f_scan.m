...
waitfor(h);
prompt = ["Enter the name of the file (.xlsx only): ","Enter the starting enthalpy of the protien,(cal/mol): "];
dlgtitle = 'Input';
dims = [1 35];
definput = {'L1F0216','32735'};
answer = inputdlg(prompt,dlgtitle,dims,definput);
%% Initialization
file_name=string(answer(1));
dH=double(string(answer(2)));%starting value
T=readtable(file_name);
min_temp=table2array(T(1,2));
max_temp=table2array(T(1,end));
min_wave=T.Var1(end);
max_wave=T.Var1(2);
...
%% 
w=x(b);
mintemp_ind=find(table2array(T(1,:))==46);
maxtemp_ind=find(table2array(T(1,:))==80);
l_exp=table2array(T((upper_ind+1)-b,mintemp_ind:maxtemp_ind))';
t=table2array(T(1,mintemp_ind:maxtemp_ind))'+273.15;
ini=l_exp(1);
fin=l_exp(end);
...
dH=c.a
for i=1:length(t)
    l_the(i)=1/(1+exp(dH/1.987*(1/Tm-1/t(i))));
end
%%
plot(t,l_the);
